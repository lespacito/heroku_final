...
module.exports = async (client) => {
    (await pGlob(`${process.cwd()}/commands/*/*.js`)).map(async (cmdFile) => {
        ...
        if (!cmd.category) return Logger.warn(`Commande non-chargée: pas de catégories ⬇\nFichier -> ${cmdFile}`);

        if (!cmd.permissions) return Logger.warn(`Commande non-chargée: pas de permissions ⬇\nFichier -> ${cmdFile}`);

        if (cmd.ownerOnly === undefined) return Logger.warn(`Commande non-chargée: pas de ownerOnly ⬇\nFichier -> ${cmdFile}`);

        if (!cmd.usage) return Logger.warn(`Commande non-chargée: pas de usage ⬇\nFichier -> ${cmdFile}`);

        if (!cmd.examples) return Logger.warn(`Commande non-chargée: pas d'examples ⬇\nFichier -> ${cmdFile}`);
